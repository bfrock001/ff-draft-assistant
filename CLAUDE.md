...
Modules: `config.py` (§1), `scoring.py` (§4), `ids.py` (§5), `loaders.py` (§3 + §5 gate), `espn_adp.py` / `projections.py` (§3 snapshots → espn_adp + proj_points), `board.py` (§6 unified table), `pool.py` (§9 NumPy player pool), `vona.py` (§8 fallback / Phase 2 VONA engine), `draft_state.py` + `app.py` (§10 Streamlit board + VONA cards). **Phases 0–2 complete** (Phase 2 tagged `v0.2.0`) — 45 tests pass; zero unmatched in top 200; full 160-pick mock over the real board with save/resume; VONA returns a sane top-3 in ~1 ms (TE-cliff test passes). `data/cache/*.parquet` and `state/*.json` are gitignored (regenerated); `data/manual_id_overrides.csv` is hand-maintained input.
